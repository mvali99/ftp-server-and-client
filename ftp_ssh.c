...
void download_file(int user_index, char* filename) {
    ...
    char log_message[100];
    sprintf(log_message, "User %s downloaded file %s.", users[user_index].username, filename);
    write_log(log_message);
}
...
void* handle_client(void* new_socket_ptr) {
	...
	if (SSL_set_fd(ssl, new_socket) == 0) {
		printf("Error setting socket for SSL.\n");
		ERR_print_errors_fp(stderr);
		exit(7);
	}
        ...
        while(1) {
            ...
            if(user_index != -1) {
                if(strcmp(command, "PWD") == 0) {
                    ...
                    SSL_write(ssl , "501 Syntax error in parameters or arguments.\n" , strlen("501 Syntax error in parameters or arguments.\n"));
                }
            } else {
                SSL_write(ssl , "530 Login incorrect.\n" , strlen("530 Login incorrect.\n"));
            }
